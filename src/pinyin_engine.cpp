...
bool PinyinEngine::processInputKey(guint keyval, guint modifiers) {
    ...
    if (keyval >= 'a' && keyval <= 'z') {
        ...
        LOG_DEBUG("Context after typing: size={} input={}",
                  context_->size(), context_->userInput());
        updateUI();
        return TRUE;
    }
    ...
    if (keyval >= '0' && keyval <= '9' && context_->size() > 0) {
        ...
        LOG_DEBUG("Number key pressed: {} -> candidate index {}",
                  keyval - '0', index);
...
void PinyinEngine::selectCandidate(size_t index) {
    ...
    LOG_DEBUG("selectCandidate: index={} total_candidates={}",
    ...
    if (index < candidates.size()) {
        ...
        LOG_WARN("Invalid candidate index: {} (total: {})",
                 index, candidates.size());
    }
}
...
void PinyinEngine::updateLookupTable() {
    ...
    LOG_DEBUG("Showing candidates {} to {} (page {})",
              start, end - 1, current_page_);

    for (size_t i = start; i < end; ++i) {
        const auto &candidate = candidates[i];
        std::string text = candidate.toString();
        LOG_DEBUG("  Candidate {}: {} (score: {})",
                  i + 1, text, candidate.score());
...
std::string PinyinEngine::convertPunctuation(guint keyval) {
    ...
    if (it != punct_map.end()) {
        LOG_DEBUG("Converting punctuation: {} -> {}",
                  (char)keyval, it->second);
        return it->second;
    }
...
void PinyinEngine::initProperties() {
    ...
    mode_prop_ = ibus_property_new("InputMode", 
                                   PROP_TYPE_NORMAL,
                                   label,
