...
void PinyinEngine::initializeSharedIME() {
    ...
    std::string lm_path = getDataPath("sc.lm");
    LOG_INFO("Language model path: " << lm_path);

    shared_ime_ = std::make_shared<PinyinIME>(
        std::make_unique<PinyinDictionary>(),
        std::make_unique<UserLanguageModel>(lm_path.c_str()));
...
std::string PinyinEngine::getDataPath(const std::string &filename) {
    ...
    return std::string("/usr/share/libime/") + filename;
}
...
bool PinyinEngine::processKeyEvent(guint keyval, guint keycode,
                                   guint modifiers) {
    LOG_DEBUG("processKeyEvent: keyval=0x"
              << std::hex << keyval << std::dec << " keycode=" << keycode
              << " modifiers=0x" << std::hex << modifiers << std::dec);
    ...
    if (is_shift && (modifiers & IBUS_RELEASE_MASK)) {
        ...
        if (shift_pressed_) {
            if (context_->size() > 0) {
                LOG_INFO(
                    "Shift released with pending input, committing raw input: "
                    << context_->userInput());
    ...
    LOG_DEBUG(
        "Input key processed: " << (result ? "handled" : "passed through"));
    return result;
}
...
bool PinyinEngine::processInputKey(guint keyval, guint modifiers) {
    ...
    if (keyval >= 'a' && keyval <= 'z') {
        ...
        LOG_DEBUG("Context after typing: size=" << context_->size() << " input="
                                                << context_->userInput());
        updateUI();
        return TRUE;
    }
    ...
    if (keyval >= '0' && keyval <= '9' && context_->size() > 0) {
        ...
        LOG_DEBUG("Number key pressed: " << (keyval - '0')
...
void PinyinEngine::selectCandidate(size_t index) {
    ...
    LOG_DEBUG("selectCandidate: index=" << index << " total_candidates="
                                        << candidates.size());
    ...
    if (index < candidates.size()) {
        ...
        LOG_WARN("Invalid candidate index: " << index << " (total: "
                                             << candidates.size() << ")");
    }
}
...
void PinyinEngine::updateLookupTable() {
    ...
    LOG_DEBUG("updateLookupTable: " << candidates.size() << " candidates");
    ...
    LOG_DEBUG("Showing candidates " << start << " to " << (end - 1) << " (page "
                                    << current_page_ << ")");

    for (size_t i = start; i < end; ++i) {
        const auto &candidate = candidates[i];
        std::string text = candidate.toString();
        LOG_DEBUG("  Candidate " << (i + 1) << ": " << text
...
std::string PinyinEngine::convertPunctuation(guint keyval) {
    ...
    if (it != punct_map.end()) {
        LOG_DEBUG("Converting punctuation: " << (char)keyval << " -> "
                                             << it->second);
        return it->second;
    }
...
void PinyinEngine::focusInId(const gchar *object_path, const gchar *client) {
    LOG_INFO("Focus in with ID: " << object_path << " client: " << client);
...
void PinyinEngine::initProperties() {
    ...
    // Create mode property with icon
    mode_prop_ = ibus_property_new("InputMode", PROP_TYPE_NORMAL,
                                   ibus_text_new_from_string("中文"),
                                   "/usr/share/ibus-libpinyin/icons/simp-chinese.svg", // Default icon
                                   ibus_text_new_from_string("切换中英文"),
                                   TRUE, // sensitive
                                   TRUE, // visible
...
void PinyinEngine::updateModeProperty() {
    ...
    // Update label and icon based on mode
    const char *label = english_mode_ ? "En" : "中";
    const char *icon =
        english_mode_ ? "input-keyboard-symbolic" : "ibus-pinyin";

    ibus_property_set_label(mode_prop_, ibus_text_new_from_string(label));
    // ibus_property_set_icon(mode_prop_, icon);
    ...
    LOG_DEBUG("Mode property updated: " << label << " (" << icon << ")");
}
